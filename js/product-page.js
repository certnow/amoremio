...
import { loadProduct } from "./products.js?v=20260730-4";
import { escapeHtml, money, productPrice } from "./utils.js";
import { initStoreShell } from "./store-shell.js?v=20260730-5";
import { discountPercent, materialLabel, publicProductTitle } from "./product-commercial.js?v=20260730-1";
...
function render(product) {
  const title=publicProductTitle(product),discount=discountPercent(product);
  document.title = `${title} | Amoremio`;
  const images = [...(product.product_images || [])].sort((a, b) => Number(b.is_primary) - Number(a.is_primary) || a.position - b.position);
  const variants = product.product_variants || [];
  const baseStock = Number(product.stock || 0);
  root.innerHTML = `${galleryMarkup(images, title)}
  <div class="product-info"><a class="breadcrumb" href="produtos.html">Coleções / ${escapeHtml(product.categories?.name || "Amoremio")}</a>${discount>=5?`<span class="product-sale-label">Saldo · ${discount}% de desconto</span>`:""}<h1>${escapeHtml(title)}</h1><p class="product-sku">SKU ${escapeHtml(product.sku)}</p><p class="product-price" data-price>${product.promotional_price ? `<s>${money(product.price)}</s> ` : ""}<strong>${money(product.promotional_price ?? product.price)}</strong></p><p class="product-description">${escapeHtml(product.description || "Informações desta peça serão adicionadas em breve.")}</p><div class="product-meta"><div><span>Material</span>${escapeHtml(materialLabel(product)||"A informar")}</div><div><span>Medidas</span>${escapeHtml(product.dimensions || "A informar")}</div></div>${product.care_instructions?`<section class="product-care"><h2>Cuidados com a peça</h2><p>${escapeHtml(product.care_instructions).replace(/\n/g,"<br>")}</p></section>`:""}
  <form class="product-form" data-product-form>${variants.length ? `<div class="field"><label for="variant">Variação</label><select id="variant" name="variant" required><option value="">Escolha uma opção</option>${variants.map((variant) => `<option value="${variant.id}" data-stock="${variant.stock}">${escapeHtml(variant.name)}: ${escapeHtml(variant.value)}${Number(variant.price_adjustment) ? ` (${Number(variant.price_adjustment) > 0 ? "+" : ""}${money(variant.price_adjustment)})` : ""}</option>`).join("")}</select></div>` : ""}<div class="product-form__row"><div class="field"><label for="quantity">Quantidade</label><input id="quantity" name="quantity" type="number" min="1" max="${baseStock}" value="1"></div><button class="button" type="submit" ${baseStock <= 0 ? "disabled" : ""}>${baseStock <= 0 ? "Esgotado" : "Adicionar ao carrinho"}</button></div><span class="stock-note" data-stock>${baseStock > 0 ? `${baseStock} unidade${baseStock === 1 ? "" : "s"} ${baseStock === 1 ? "disponível" : "disponíveis"}` : "Produto esgotado"}</span><p class="product-message" data-message aria-live="polite"></p></form></div>`;
  ...
  variantSelect?.addEventListener("change", () => { const variant = variants.find((item) => item.id === variantSelect.value); const stock = variant ? Number(variant.stock) : baseStock; quantity.max = stock; quantity.value = Math.min(Number(quantity.value), stock) || 1; stockNode.textContent = `${stock} unidade${stock === 1 ? "" : "s"} ${stock === 1 ? "disponível" : "disponíveis"}`; priceNode.innerHTML = `<strong>${money(productPrice(product, variant))}</strong>`; if (variant?.image_id) gallery.showByReference(variant.image_id); else if (variant?.image_url) gallery.showByReference(variant.image_url); });
  form.addEventListener("submit", (event) => { event.preventDefault(); const variant = variants.find((item) => item.id === variantSelect?.value); if (variants.length && !variant) return form.querySelector("[data-message]").textContent = "Escolha uma variação."; const stock = variant ? Number(variant.stock) : baseStock; addToCart({ productId: product.id, slug: product.slug, sku:product.sku, material:materialLabel(product), name:title, imageUrl: variant?.image_url || images[0]?.image_url || "", variantId: variant?.id || null, variantLabel: variant ? `${variant.name}: ${variant.value}` : "", unitPrice: productPrice(product, variant), quantity: Number(quantity.value), stock }); form.querySelector("[data-message]").innerHTML = 'Produto adicionado. <a href="carrinho.html">Ver carrinho</a>'; });
}
